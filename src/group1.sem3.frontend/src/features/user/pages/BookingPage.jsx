...
export default function BookingPage() { 
    ...
    function useRooms(companyId) {
        ...
        useEffect(() => {
            ...
            (async () => {
                try { 
                    const data = await getRooms(companyId);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await getDesksForRoom(COMPANY_ID, selectedRoom.id);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const qs = { startDate: dayStartUTC, endDate: dayEndUTC };
                const data = await getReservations(COMPANY_ID, qs);
    ...
    async function handleBook() {
        ...
        try {
            await createReservation(COMPANY_ID, payload);
